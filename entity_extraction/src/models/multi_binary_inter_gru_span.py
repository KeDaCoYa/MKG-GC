...
logger = logging.getLogger("main.MultiBinaryInterGRUSpanForEight")

class MultiBinaryInterGRUSpanForEight(BaseBert):
    def __init__(self,config):
        super(MultiBinaryInterGRUSpanForEight, self).__init__(config)
        ...
        self.chem_drug_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.gene_protein_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.disease_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.cell_line_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.cell_type_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.dna_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.rna_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
        self.species_mid_linear = nn.GRU(out_dims, mid_linear_dims, batch_first=True, bidirectional=True, num_layers=2,dropout=0.5)
    ...
    def forward(self, token_ids, attention_masks, token_type_ids, input_token_starts=None, start_ids=None, end_ids=None,input_true_length=None,entity_type_ids=None):
        ...
        if self.config.bert_name in ['biobert','wwm_bert','flash','flash_quad','bert','scibert']:
        ...
        if entity_type_ids[0][0].item() == 0:
            ...
            dise_sequence_output = (dise_sequence_output + disease_inter_logits)
            ...
            chem_sequence_output = (chem_sequence_output + chem_inter_logits)
            ...
            gene_sequence_output = (gene_sequence_output + gene_inter_logits)
            ...
            spec_sequence_output = (spec_sequence_output + spec_inter_logits)
            ...
            cellline_sequence_output = (cellline_sequence_output + cellline_inter_logits)
            ...
            dna_sequence_output = (dna_sequence_output + dna_inter_logits)
            ...
            rna_sequence_output = (rna_sequence_output + rna_inter_logits)
            ...
            celltype_sequence_output = (celltype_sequence_output + celltype_inter_logits)
            ...
            dise_sequence_output = F.relu(self.disease_mid_linear(dise_sequence_output)[0])  # disease logit value
            chem_sequence_output = F.relu(self.chem_drug_mid_linear(chem_sequence_output)[0])  # chemical logit value
            gene_sequence_output = F.relu(self.gene_protein_mid_linear(gene_sequence_output)[0])  # gene/protein logit value
            spec_sequence_output = F.relu(self.species_mid_linear(spec_sequence_output)[0])  # species logit value
            cellline_sequence_output = F.relu(self.cell_line_mid_linear(cellline_sequence_output)[0])  # cell line logit value
            dna_sequence_output = F.relu(self.dna_mid_linear(dna_sequence_output)[0])  # dna logit value
            rna_sequence_output = F.relu(self.rna_mid_linear(rna_sequence_output)[0])  # rna logit value
            celltype_sequence_output = F.relu(self.cell_type_mid_linear(celltype_sequence_output)[0])  # cell type logit value
            ...
            cellline_sequence_output = (cellline_sequence_output + cell_line_inter_logits)
            cellline_end_logits = self.cell_line_end_fc(cellline_sequence_output)  # cell line logit value
