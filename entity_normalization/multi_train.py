...
from multi_biosyn_evaluate import dev, test
...
from src.models.multi_biosyn import MultiBioSyn, MyRerankNet
...
from utils.function_utils import get_config, get_logger, set_seed, save_model, load_model_and_parallel, \
    print_hyperparameters, count_parameters
...
def biosyn_train(config, logger):
    ...
    if config.debug:  # 开启debug，使用小部分数据集进行测试
        bc5cdr_disease_train_dictionary = bc5cdr_disease_train_dictionary[:1000]
        bc5cdr_chemical_train_dictionary = bc5cdr_chemical_train_dictionary[:1000]
        ncbi_disease_train_dictionary = ncbi_disease_train_dictionary[:1000]
    ...
    avg_count = max(len(ncbi_disease_train_mentions),len(bc5cdr_chemical_train_mentions),len(bc5cdr_chemical_train_mentions))
    #avg_count = (len(ncbi_disease_train_mentions)+len(bc5cdr_chemical_train_mentions)+len(bc5cdr_chemical_train_mentions))//3
    if len(ncbi_disease_train_mentions)>avg_count:
        ...
        for i in range(avg_count-len(ncbi_disease_train_mentions)):
            idx = random.randint(0,len(ncbi_disease_train_mentions))
    ...
    if len(bc5cdr_disease_train_mentions)>avg_count:
        ...
        for i in range(avg_count-len(bc5cdr_disease_train_mentions)):
            idx = random.randint(0, len(bc5cdr_disease_train_mentions))
    ...
    if len(bc5cdr_chemical_train_mentions)>avg_count:
        ...
        for i in range(avg_count-len(bc5cdr_chemical_train_mentions)):
            idx = random.randint(0, len(bc5cdr_chemical_train_mentions))
    ...
    for epoch in range(1, config.num_epochs + 1):
        ...
        model.train()
        flag = 0
        if len(bc5cdr_disease_train_dataloader)>len(bc5cdr_chemical_train_dataloader):
            flag = 0
        else:
            flag = 1

        for step, data in tqdm(enumerate(zip(bc5cdr_disease_train_dataloader, bc5cdr_chemical_train_dataloader,ncbi_disease_train_dataloader)),desc="正在训练模型...."):

            #model.optimizer.zero_grad()
            optimizer.zero_grad()
            # batch_y就是label，为CUI
            bc5cdr_disease_data,bc5cdr_chemcial_data,ncbi_disease_data = data
            ...
            loss = bc5cdr_disease_loss + bc5cdr_chemical_loss+ncbi_disease_loss
            ...
            logger.info(
                'Epoch:{} 训练中>>>>>> {}/{} loss:{:5f},bc5dis_loss:{:.5f},bc5chem_loss:{:.5f},ncbi_loss:{:.5f} ,lr={}'.format(epoch, global_step, t_total,
                                                                                         loss.item(),
                                                                                         bc5cdr_disease_loss.item(),
                                                                                         bc5cdr_chemical_loss.item(),
                                                                                         ncbi_disease_loss.item(),
                                                                                         lr))
            logger.info("   disease sparse weight:{:.5f}".format(bc5cdr_disease_sparse_weight))
        ...
        if config.use_wandb:
            if config.use_scheduler:
                if config.freeze_bert:
                    wandb_name = f'三任务_{config.model_name}_epochs_{config.num_epochs}_encoder_{config.encoder_type}{config.task_encoder_nums}_scheduler{config.warmup_proportion}_lr{config.learning_rate}_freeze_{len(config.freeze_layers)}_bs{config.batch_size}_maxlen{config.max_len}'
                else:
                    wandb_name = f'三任务_{config.model_name}_epochs_{config.num_epochs}_encoder_{config.encoder_type}{config.task_encoder_nums}_scheduler{config.warmup_proportion}_lr{config.learning_rate}_no_freeze_bs{config.batch_size}_maxlen{config.max_len}'
            else:
                if config.freeze_bert:
                    wandb_name = f'三任务_{config.model_name}_epochs_{config.num_epochs}_encoder_{config.encoder_type}_{config.task_encoder_nums}_lr{config.learning_rate}_freeze_{len(config.freeze_layers)}_bs{config.batch_size}_maxlen{config.max_len}'
                else:
                    wandb_name = f'三任务_{config.model_name}_epochs_{config.num_epochs}_encoder_{config.encoder_type}_{config.task_encoder_nums}_lr{config.learning_rate}_no_freeze_bs{config.batch_size}_maxlen{config.max_len}'
