...
class DenseEncoder(nn.Module):
    def __init__(self, config):
        ...
        self.dense_encoder = AutoModel.from_pretrained(config.bert_dir)

        self.mid_linear = nn.Linear(config.hidden_size,300)

    def forward(self, input_ids=None,attention_mask=None,token_type_ids=None):

        seq_out = self.dense_encoder(input_ids=input_ids,attention_mask=attention_mask,token_type_ids=token_type_ids)
        cls = seq_out[0][:,0]

        output = self.mid_linear(cls)
        return output
...
class BioSynLite:
    ...
    def load_dense_encoder(self, bert_dir):
        ...
        """
        self.dense_encoder = DenseEncoder(self.config)
    ...
    def get_dense_representation(self,mentions,verbose=False,type_=None):
        '''
        ...
        with torch.no_grad():
            for batch in tqdm(name_dataloader, disable=not verbose, desc='get dense representation'):
                ...
                batch_dense_embeds = self.dense_encoder(**batch).cpu().detach().numpy()
                dense_embeds.append(batch_dense_embeds)
...
class RerankNet(nn.Module):
    ...
    def forward(self, x):
        ...
        query_embed = query_embed.unsqueeze(1)  # query : [batch_size, 1, hidden]
        ...
        candidate_embeds = candidate_embeds.reshape(batch_size, topk, -1)  # [batch_size, topk, hidden]
