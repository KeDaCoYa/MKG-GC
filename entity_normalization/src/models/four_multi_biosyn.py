...
class BaseEncoder(nn.Module):
    ...
    def forward(self,input_ids=None,attention_mask=None,token_type_ids=None,type_=0):
        ...
        :param type_: 这个表示对于不同类别的数据，使用不同的模型
        :return:
        """
        base_output = self.bert_model(input_ids=input_ids,attention_mask=attention_mask,token_type_ids=token_type_ids)
...
class MultiEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.encoder_type == 'bert':
            self.encoder_class = BertLayer
        elif config.encoder_type == 'gau':
            self.encoder_class = GAU
        elif config.encoder_type == 'mlp':
            pass


        if config.encoder_type in ['bert','gau']:
            # disease encoder
            self.bc5cdr_disease_encoder = nn.ModuleList(
                [self.encoder_class(config) for _ in range(self.config.task_encoder_nums)])
            # chem encoder
            self.bc5cdr_chemical_encoder = nn.ModuleList(
                [self.encoder_class(config) for _ in range(self.config.task_encoder_nums)])
            self.ncbi_disease_encoder = nn.ModuleList(
                [self.encoder_class(config) for _ in range(config.task_encoder_nums)])
            self.bc2gm_encoder = nn.ModuleList(
                [self.encoder_class(config) for _ in range(config.task_encoder_nums)])
        elif config.encoder_type in ['mlp']:
            self.bc5cdr_disease_encoder = nn.ModuleList(
                [nn.Linear(config.hidden_size,500),nn.ReLU(),nn.Linear(500,300),nn.ReLU(),nn.Linear(300,config.hidden_size),nn.ReLU()]
            )
            self.bc5cdr_chemical_encoder = nn.ModuleList(
                [nn.Linear(config.hidden_size, 500), nn.ReLU(), nn.Linear(500, 300), nn.ReLU(),
                 nn.Linear(300, config.hidden_size), nn.ReLU()]
            )
            self.ncbi_disease_encoder = nn.ModuleList(
                [nn.Linear(config.hidden_size, 500), nn.ReLU(), nn.Linear(500, 300), nn.ReLU(),
                 nn.Linear(300, config.hidden_size), nn.ReLU()]
    ...
    def forward(self,input_ids=None,attention_mask=None,token_type_ids=None,type_=0):
        ...
        base_output = self.bert_model(input_ids=input_ids,attention_mask=attention_mask,token_type_ids=token_type_ids)[0]
        if self.config.encoder_type == 'bert':
            attention_mask = attention_mask.unsqueeze(-2).unsqueeze(-2)
        if self.config.encoder_type in ['bert', 'gau']:
            if type_ == 0:
                for i in range(self.config.task_encoder_nums):

                    base_output = self.bc5cdr_disease_encoder[i](base_output,attention_mask=attention_mask)[0]

            elif type_ == 1:
                for i in range(self.config.task_encoder_nums):
                    base_output = self.bc5cdr_chemical_encoder[i](base_output,attention_mask=attention_mask)[0]

            elif type_ == 2:
                for i in range(self.config.task_encoder_nums):
                    base_output = self.ncbi_disease_encoder[i](base_output,attention_mask=attention_mask)[0]
            elif type_ == 3:
                for i in range(self.config.task_encoder_nums):
                    base_output = self.bc2gm_encoder[i](base_output,attention_mask=attention_mask)[0]
        else:
            if type_ == 0:
                base_output = self.bc5cdr_disease_encoder(base_output)
            elif type_ == 1:
                base_output = self.bc5cdr_chemical_encoder(base_output)
            elif type_ == 2:
                base_output = self.ncbi_disease_encoder(base_output)
            elif type_ == 3:
                base_output = self.bc2gm_encoder(base_output)


        return (base_output,)

class MultiBioSynFour:
    def __init__(self,config,device,initial_sparse_weight=None):
        ...
        self.bc5cdr_disease_sparse_weight = None
        self.bc5cdr_chem_sparse_weight = None
        self.ncbi_disease_sparse_weight = None
        self.bc2gm_sparse_weight = None


        if initial_sparse_weight != None:
            self.init_sparse_weight(initial_sparse_weight)
    ...
    def init_sparse_weight(self, initial_sparse_weight):
        ...
        self.bc5cdr_disease_sparse_weight = nn.Parameter(torch.empty(1).to(self.device))
        self.bc5cdr_disease_sparse_weight.data.fill_(initial_sparse_weight)

        self.bc5cdr_chem_sparse_weight = nn.Parameter(torch.empty(1).to(self.device))
        self.bc5cdr_chem_sparse_weight.data.fill_(initial_sparse_weight)

        self.ncbi_disease_sparse_weight = nn.Parameter(torch.empty(1).to(self.device))
        self.ncbi_disease_sparse_weight.data.fill_(initial_sparse_weight)

        self.bc2gm_sparse_weight = nn.Parameter(torch.empty(1).to(self.device))
        self.bc2gm_sparse_weight.data.fill_(initial_sparse_weight)

        return self.bc5cdr_disease_sparse_weight,self.bc5cdr_chem_sparse_weight,self.ncbi_disease_sparse_weight,self.bc2gm_sparse_weight
    ...
    def init_bc5cdr_disease_sparse_encoder(self, corpus):
        '''
        初始化使用tf-idf计算sparse representation
        :param corpus:
        :return:
        '''
        self.bc5cdr_disease_sparse_encoder = SparseEncoder(self.device).fit(corpus)

        return self.bc5cdr_disease_sparse_encoder

    def init_bc5cdr_chem_sparse_encoder(self, corpus):
        '''
        初始化使用tf-idf计算sparse representation
        :param corpus:
        :return:
        '''
        self.bc5cdr_chem_sparse_encoder = SparseEncoder(self.device).fit(corpus)

        return self.bc5cdr_chem_sparse_encoder
    def init_ncbi_disease_sparse_encoder(self, corpus):
        '''
        初始化使用tf-idf计算sparse representation
        :param corpus:
        :return:
        '''
        self.ncbi_disease_sparse_encoder = SparseEncoder(self.device).fit(corpus)

        return self.ncbi_disease_sparse_encoder
    def init_bc2gm_sparse_encoder(self, corpus):
        '''
        初始化使用tf-idf计算sparse representation
        :param corpus:
        :return:
        '''
        self.bc2gm_sparse_encoder = SparseEncoder(self.device).fit(corpus)

        return self.bc2gm_sparse_encoder
    ...
    def get_bc5cdr_disease_sparse_representation(self,mentions,verbose=False):
        '''
        ...
        for start in tqdm(range(0, len(mentions), batch_size),disable=not verbose,desc='get bc5cdr-disease sparse embedding'):
            end = min(start + batch_size, len(mentions))
            batch = mentions[start:end]
            # 调用tf-ids的transform，得到
            batch_sparse_embeds = self.bc5cdr_disease_sparse_encoder(batch)
    ...
    def get_bc5cdr_chemical_sparse_representation(self,mentions,verbose=False):
        '''
        将数据集中的mention，使用sparse encoder进行编码，得到sparse representations
        :param mentions:
        :param verbose:
        ...
        for start in tqdm(range(0, len(mentions), batch_size),disable=not verbose,desc='get bc5cdr-chemical sparse embedding'):
            end = min(start + batch_size, len(mentions))
            batch = mentions[start:end]
            # 调用tf-ids的transform，得到
            batch_sparse_embeds = self.bc5cdr_chem_sparse_encoder(batch)
            batch_sparse_embeds = batch_sparse_embeds.cpu().numpy()
            sparse_embeds.append(batch_sparse_embeds)
        sparse_embeds = np.concatenate(sparse_embeds, axis=0)

        return sparse_embeds
    def get_bc2gm_sparse_representation(self,mentions,verbose=False):
        '''
        将数据集中的mention，使用sparse encoder进行编码，得到sparse representations
        :param mentions:
        :param verbose:
        ...
        for start in tqdm(range(0, len(mentions), batch_size),disable=not verbose,desc='get bc2gm sparse embedding'):
            end = min(start + batch_size, len(mentions))
            batch = mentions[start:end]
            # 调用tf-ids的transform，得到
            batch_sparse_embeds = self.bc2gm_sparse_encoder(batch)
            batch_sparse_embeds = batch_sparse_embeds.cpu().numpy()
            sparse_embeds.append(batch_sparse_embeds)
        sparse_embeds = np.concatenate(sparse_embeds, axis=0)

        return sparse_embeds
    def get_ncbi_disease_sparse_representation(self,mentions,verbose=False):
        '''
        将数据集中的mention，使用sparse encoder进行编码，得到sparse representations
        :param mentions:
        :param verbose:
        ...
        for start in tqdm(range(0, len(mentions), batch_size),disable=not verbose,desc='get ncbi_disease sparse embedding'):
            end = min(start + batch_size, len(mentions))
            batch = mentions[start:end]
            # 调用tf-ids的transform，得到
            batch_sparse_embeds = self.ncbi_disease_sparse_encoder(batch)
            batch_sparse_embeds = batch_sparse_embeds.cpu().numpy()
            sparse_embeds.append(batch_sparse_embeds)
        sparse_embeds = np.concatenate(sparse_embeds, axis=0)
    ...
    def get_dense_representation(self,mentions,verbose=False,type_=0):
        ...
        dense_embeds = []
        if type_ == 0:
            desc = 'get bc5cdr disease dense representation'
        elif type_ == 1:
            desc = 'get bc5cdr chem dense representation'
        elif type_ == 2:
            desc = 'get ncbi disease dense representation'
        elif type_ == 3:
            desc = 'get bc2gm dense representation'

        with torch.no_grad():
            for batch in tqdm(name_dataloader,total=len(name_dataloader), disable=not verbose, desc=desc):
...
class MyRerankNetFour(nn.Module):
    """
        这个用于多任务的RerankNet
    """
    def __init__(self,config, dense_encoder,sparse_weight,device):
        ...
        super(MyRerankNetFour, self).__init__()
        self.dense_encoder = dense_encoder
        self.config = config

        self.device = device
        self.bc5cdr_disease_sparse_weight,self.bc5cdr_chem_sparse_weight, self.ncbi_disease_sparse_weight,self.bc2gm_sparse_weight = sparse_weight
    ...
    def forward(self, x,type_=0):
        ...
        # TODO:之后3 sparse weight需要进行修改
        if type_ == 0:
            score = self.bc5cdr_disease_sparse_weight * candidate_s_scores + candidate_d_score
        elif type_ == 1:
            score = self.bc5cdr_chem_sparse_weight * candidate_s_scores + candidate_d_score
        elif type_ == 2:
            score = self.ncbi_disease_sparse_weight * candidate_s_scores + candidate_d_score
        elif type_ == 3:
            score = self.bc2gm_sparse_weight * candidate_s_scores + candidate_d_score
        # todo:这是只用1个sparse weight
        # score = self.bc5cdr_disease_sparse_weight * candidate_s_scores + candidate_d_score
