...
def dev(model, config,device, type_weight,ckpt_path='/root/code/biobert_task/ner/output/best_model/model.pt',word2id=None,tokenizer=None, metric=None,logger=None,epoch=None,global_step=None,type_='dev'):
    ...
    dev_dataset, dev_loader = choose_dataset_and_loader(config, dev_data, dev_labels, tokenizer, word2id,type_='dev')
    ...
    with torch.no_grad():
        for step, batch_data in tqdm(enumerate(dev_loader)):
            ...
            if model_name == 'bert_crf' or model_name == 'bert_bilstm_crf' or model_name == 'bert_mlp':
                ...
                dev_predicate_li.extend(predicate_label_BIO)
                dev_labels_li.extend(true_label_BIO)
    ...
    if config.decoder_layer == 'span':
        ...
        try:
            ...
            all_dev_p = precision_score(true_label_BIOs, predicate_label_BIOs)
            all_dev_r = recall_score(true_label_BIOs, predicate_label_BIOs)
            all_dev_f1 = f1_score(true_label_BIOs, predicate_label_BIOs)
...
if __name__ == '__main__':
    ...
    if config.run_type == 'normal':
        ckpt_path = '/root/code/bioner/ner/outputs/save_models/bert_mlp/jnlpba/best_model/model.pt'
        ...
        dev(model=model, config=config, device=device, type_weight=None, ckpt_path=ckpt_path,word2id=None, tokenizer=tokenizer, metric=None, logger=logger, epoch=0, global_step=0)
