...
def multi_predict_topk(biosyn:MultiBioSyn, eval_dictionary, eval_queries, topk, score_mode='hybrid',type_=0):
    """
    开始预测模型
    ----------
    score_mode : str
        hybrid, dense, sparse
    """
    bc5cdr_disease_sparse_weight, bc5cdr_chem_sparse_weight, ncbi_disease_sparse_weight = biosyn.get_sparse_weight()
    if type_ == 0:
...
def evaluate(biosyn, eval_dictionary, eval_queries, topk, score_mode='hybrid',type_=0):
    ...
    return result
def dev(config:MyBertConfig,logger,ckpt_path=None,biosyn=None,device=None,wandb=None,epoch=None,global_step=None):
    '''
    ...
    '''
    # 加载验证集/测试集 的字典和数据集
    bc5cdr_disease_eval_dictionary = load_dictionary(config.bc5cdr_disease_dev_dictionary_path)
    bc5cdr_disease_eval_queries = load_my_data(config.bc5cdr_disease_dev_path)

    bc5cdr_chemical_eval_dictionary = load_dictionary(config.bc5cdr_chemical_dev_dictionary_path)
    bc5cdr_chemical_eval_queries = load_my_data(config.bc5cdr_chemical_dev_path)

    ncbi_disease_eval_dictionary = load_dictionary(config.ncbi_disease_dev_dictionary_path)
    ncbi_disease_chemical_eval_queries = load_my_data(config.ncbi_disease_dev_path)
    ...
    logger.info('-------bc5cdr-disease----------')
    logger.info("acc@1={}".format(bc5cdr_disease_result_evalset['acc1']))
    logger.info("acc@5={}".format(bc5cdr_disease_result_evalset['acc5']))

    logger.info('-------bc5cdr-chemical----------')
    logger.info("acc@1={}".format(bc5cdr_chemical_result_evalset['acc1']))
    logger.info("acc@5={}".format(bc5cdr_chemical_result_evalset['acc5']))

    logger.info('-------ncbi-disease----------')
    logger.info("acc@1={}".format(ncbi_disease_result_evalset['acc1']))
    logger.info("acc@5={}".format(ncbi_disease_result_evalset['acc5']))
    if config.use_wandb:
        wandb.log(
            {"dev-epoch": epoch,
             'dev-bc5cdr-disease-hit@1': bc5cdr_disease_result_evalset['acc1'],
             'dev-bc5cdr-chemical-hit@1': bc5cdr_chemical_result_evalset['acc1'],
             'dev-ncbi-disease-hit@1': ncbi_disease_result_evalset['acc1'],
             'dev-bc5cdr-disease-hit@5': bc5cdr_disease_result_evalset['acc5'],
             'dev-bc5cdr-chemical-hit@5': bc5cdr_chemical_result_evalset['acc5'],
             'dev-ncbi_disease-hit@5': ncbi_disease_result_evalset['acc5'],
             }, step=global_step)
