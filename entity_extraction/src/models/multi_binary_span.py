...
class MultiBinarySpanForEight(BaseBert):
    def __init__(self,config):
        super(MultiBinarySpanForEight, self).__init__(config)
...
class MultiBinarySpanForBinary(BaseBert):
    def __init__(self, config):
        super(MultiBinarySpanForBinary, self).__init__(config)
        # 这个时候numtags=2，因为只有disease一种类别
        self.config = config
        self.num_tags = 2
        out_dims = self.bert_config.hidden_size
    ...
    def forward(self, token_ids, attention_masks, token_type_ids, input_token_starts=None, start_ids=None, end_ids=None,
        ...
        :return:
        """
        # gene_start_ids = (start_ids==2).long()
        # gene_end_ids = (end_ids==2).long()
        # disease_start_ids = (start_ids == 1).long()
        # disease_end_ids = (end_ids == 1).long()
        start_ids = (start_ids != 0).long()
        end_ids = (end_ids != 0).long()
...
class MultiBinarySpanForBinaryV2(BaseBert):
    def __init__(self, config):
        super(MultiBinarySpanForBinaryV2, self).__init__(config)
        # 这个时候numtags=2，因为只有disease一种类别
        self.config = config
        self.num_tags = 2
        out_dims = self.bert_config.hidden_size
    ...
    def forward(self, token_ids, attention_masks, token_type_ids, input_token_starts=None, start_ids=None, end_ids=None,
        ...
        gene_start_ids = (start_ids==2).long()
        gene_end_ids = (end_ids==2).long()
        chemical_start_ids = (start_ids == 1).long()
        chemical_end_ids = (end_ids == 1).long()
        # start_ids = (start_ids != 0).long()
        # end_ids = (end_ids != 0).long()
        if self.config.bert_name in ['biobert', 'wwm_bert', 'flash_quad', 'scibert', 'bert']:
            bert_outputs = self.bert_model(input_ids=token_ids, attention_mask=attention_masks,
        ...
        if isinstance(start_logits,tuple):
            output = (start_logits[0]+start_logits[1], end_logits[0]+end_logits[1])
        else:
            output = (start_logits, end_logits)

        if start_ids is not None and end_ids is not None:  # 这是训练模式，计算loss
            loss_mask = torch.zeros((start_logits[0].shape[0], start_logits[0].shape[1])).to(token_ids.device)

            for i, lens in enumerate(input_true_length):
                loss_mask[i][:lens] = 1
            # start_logtis.shape=torch.Size([4096, 14])
            chemical_start_logits, gene_start_logits = start_logits
            chemical_end_logits, gene_end_logits = end_logits


            chemical_start_logits = chemical_start_logits.view(-1, self.num_tags)
            chemical_end_logits = chemical_end_logits.view(-1, self.num_tags)

            gene_start_logits = gene_start_logits.view(-1, self.num_tags)
            gene_end_logits = gene_end_logits.view(-1, self.num_tags)
            ...
            active_chemical_start_logits = chemical_start_logits[mask]  # (?,14)这个？的值就并不确定了
            active_chemical_end_logits = chemical_end_logits[mask]  # (?,14)这个？的值就并不确定了

            active_gene_start_logits = gene_start_logits[mask]  # (?,14)这个？的值就并不确定了
            active_gene_end_logits = gene_end_logits[mask]  # (?,14)这个？的值就并不确定了


            active_chemical_start_labels = chemical_start_ids.view(-1)[mask]
            active_chemical_end_labels = chemical_end_ids.view(-1)[mask]

            active_gene_start_labels = gene_start_ids.view(-1)[mask]
            active_gene_end_labels = gene_end_ids.view(-1)[mask]

            chemical_start_loss = self.criterion(active_chemical_start_logits, active_chemical_start_labels).mean(dim=-1)
            chemical_end_loss = self.criterion(active_chemical_end_logits, active_chemical_end_labels).mean(dim=-1)


            gene_start_loss = self.criterion(active_gene_start_logits, active_gene_start_labels).mean(dim=-1)
            gene_end_loss = self.criterion(active_gene_end_logits, active_gene_end_labels).mean(dim=-1)



            loss = chemical_start_loss + chemical_end_loss+gene_start_loss+gene_end_loss
...
class MultiBinarySpanForFive(BaseBert):
    def __init__(self, config):
        super(MultiBinarySpanForFive, self).__init__(config)
        # 这个时候numtags=2，因为只有disease一种类别
        self.config = config
        self.num_tags = 2
        out_dims = self.bert_config.hidden_size
...
class MultiBinarySpanForFour(BaseBert):
    def __init__(self, config):
        super(MultiBinarySpanForFour, self).__init__(config)
        # 针对有四个类别的模型
        self.config = config
        self.num_tags = 2
        out_dims = self.bert_config.hidden_size
